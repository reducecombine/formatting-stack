...
(deftest lint!
  (doseq [cache-type ["lru" "soft"]]
    (let [cache (util.caching/new-cache {:capacity 100
                                         :type     cache-type})
          ...
          key-fn (speced/fn [linter ^File f]
                   (is (= linter
                          wrapee)
                       "The linter forms part of the key derivation fn")
                   (condp = (-> f str)
                     "a.clj" "hash-of-a"
                     "b.clj" "hash-of-b"
      ...
      (are [desc input expected-call-count expected-cache] (testing input
                                                             (sut/lint! {::sut/wrapee wrapee
                                                                         ::sut/key-fn  key-fn
                                                                         ::sut/cache   cache}
                                                                        input)
                                                             (is (= expected-cache
                                                                    (util.caching/current-value cache)))
